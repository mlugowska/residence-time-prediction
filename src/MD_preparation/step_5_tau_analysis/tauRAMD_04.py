...
PDBS = externals.PDB_ONLY_KOKH
for pdb_id in PDBS.keys():
    for rep_num in range(1, 6):
        d_list.append(f'{externals.MOUNT_ICM_PATH}/{pdb_id}/{rep_num}/{pdb_id}_times_{rep_num}.dat')
    ...
    for t, d in enumerate(d_list):
        ...
        for r in read_data:
            try:
                if soft == "NAMD":
                    times.append(
                        int(r[r.find("EXIT:") + 6:r.find(">") - 2]))  # if NAMD  was used to generate RAMD trajectories
                else:
                    times.append(int(r[r.find("after") + 6:r.find(
                        "steps") - 1]))  # if Gromacs was used to generate RAMD trajectories
            except:
                print("Input files dont't have correct format")
                printUsage()
                sys.exit()
        #    print(times)
        times = np.asarray(times) * 2 / 1000000.
    ...
    # fig.align_labels()
    plt.savefig(f'res_times_{pdb_id}.png', bbox_inches='tight', dpi=300)
    plt.close(fig)
    ...
    plt.title(
        "Residence times for " + str(t + 1) + " replicas, mean: " + str(
            np.round(np.mean(mue_set), 2)) + "  std: " + str(
            np.round(np.std(mue_set), 2)), fontsize=10)
    plt.savefig(f'res_times_summary_{pdb_id}.png', bbox_inches='tight', dpi=300)
    print("Residence times for " + str(t + 1) + " replicas, mean: " + str(
        np.round(np.mean(mue_set), 2)) + "  std: " + str(
        np.round(np.std(mue_set), 2)))
    plt.close(fig)
