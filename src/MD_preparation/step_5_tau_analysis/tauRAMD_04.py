...
soft = "NAMD"  # if Gromacs software was used for RAMD simulations;  otherwise define  soft = 'NAMD'
...
d_list = []
if len(sys.argv) < 2:
    d_list = get_files('/data/complex_files/1XKK/ramd', '.dat')
else:
...
for t, d in enumerate(d_list):
    ...
    for r in read_data:
        if soft == "NAMD":
            ...
            times.append(
                int(r[r.find("after") + 6:r.find("steps") - 1]))  # if Gromacs was used to generate RAMD trajectories
    print(times)
    times = np.asarray(times) / 5000000.
...
for t, times in enumerate(times_set):
    ...
    if len(times) > 0:
        ...
        s = ax0.hist(times, bins=bins, cumulative=True, histtype="step", color='k')
        ...
        tau = ret_time(times)
        if index_x_half > 0:
            x_half = (s[1][index_x_half] + s[1][index_x_half - 1] / 2)
        else:
            x_half = s[1][index_x_half]
        ...
        ax1.hist(x=bt2, bins=5, alpha=0.5, density=True, histtype="stepfilled")
        ax1.set_xlim([min(bt2) - 0.00005, max(bt2) + 0.00005])
        mu, std = norm.fit(bt2)
        mue_set.append(np.round(mu, 4))
        xmin, xmax = plt.xlim()
        ymin, ymax = plt.ylim()
        ...
        xmin = min(times)
        xmax = np.round(max(times), 4)
        # if xmax == 0: xmax = 0.5
        tp = np.linspace(xmin, xmax, 100)
        # tp = np.linspace(xmin, xmax, 10)
        ...
        KS = np.round(np.max(np.abs(1 - np.exp(-(np.asarray(hist_center)) / mu) - CD)), 4)
        # import pdb; pdb.set_trace()
        plt.title("KS test:" + str(KS), fontsize=12)
        print(" Relative res. time and SD: ", np.round(mu, 4), np.round(std, 4), "KS test:", KS)
        print("-----------------------------------------------------------------")
...
plt.boxplot(times_set, showmeans=True, meanline=True, meanprops=meanpointprops, medianprops=medianpointprops,
            bootstrap=5000)  # labels = mue_set)
ymin, ymax = plt.ylim()
plt.ylim = (0, ymax)
plt.grid(linestyle='--', linewidth=0.5)
...
plt.title(
    ...
    "Residence times for " + str(t + 1) + " replica(s), mean: " + str(np.round(np.mean(mue_set), 4)) + "  std: " + str(
        np.round(np.std(mue_set), 4)), fontsize=12)
